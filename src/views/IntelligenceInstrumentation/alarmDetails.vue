      ...
      <el-row :gutter="35" style="height: 100%" class="processingBox" v-model="alarmDetails">
          ...
          <p>
            <span class="textMagin"><span class="mr20">处理状态 :</span>
              <span v-if="alarmDetails.status==0">待确认</span>
          ...
          <div class="confirmBox">
              <h3>确认信息</h3>
              <p><span class="mr35">确认人 :</span> {{alarmDetails.extend.confirmer}}</p>
              <p class="mb20"><span class="mr20">确认时间 :</span>{{alarmDetails.extend.confirmTime}}</p>
          </div>
          <div class="feedbackBox">
              <h3>反馈信息</h3>
              <p><span class="mr35">反馈人 :</span> {{alarmDetails.extend.feedbackPerson}}</p>
              <p><span class="mr20">反馈时间 :</span>{{alarmDetails.extend.feedbackTime}}</p>
              <p class="mb20"><span class="mr20">反馈信息 :</span>{{alarmDetails.extend.feedbackContent}}</p>
          </div>
...
<script>
  import moment from 'moment'
    export default {
        ...
        data() {
            return {
              alarmDetails:this.$route.params.row,
              tableData:[],
              total:0,
               filter: {
                organId: 1,
